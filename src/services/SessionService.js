...
let sessionService;
try {
  // Ensure SessionService is defined and is a constructor
  if (typeof SessionService === 'function' && SessionService.prototype) {
    sessionService = new SessionService();
  } else {
    throw new Error('SessionService is not a valid constructor');
  }
} catch (error) {
  console.error('Failed to instantiate SessionService:', error);
  
  // Create fallback service with minimal functionality
  sessionService = {
    validateSession: async () => ({ isValid: false, error: 'Service unavailable' }),
    createSession: async () => ({ success: false, error: 'Service unavailable' }),
    getCurrentSession: async () => null,
    getCurrentUser: async () => null,
    getToken: async () => null,
    updateUser: async () => ({ success: false, error: 'Service unavailable' }),
    clearSession: () => {
      try {
    ...
    addListener: () => {},
    removeListener: () => {},
    getSessionInfo: async () => ({ error: 'Service unavailable' }),
    createGuestSession: async () => ({ success: false, error: 'Service unavailable' })
  };
}
