...
class SessionService {
  ...
  async _initialize() {
    try {
      ...
      if (storedSession && !this._isSessionExpired(storedSession)) {
        this.currentSession = storedSession;
      }
  ...
  async getCurrentSession() {
    try {
      if (!this.currentSession) {
        ...
        if (stored && !this._isSessionExpired(stored)) {
  ...
  async isAuthenticated() {
    try {
      const session = await this.getCurrentSession();
      return session && session.user && !this._isSessionExpired(session);
    } catch (error) {
  ...
  async createSession(userData, token = null) {
    try {
      ...
      this._notifyListeners('sessionCreated', session);
  ...
  async validateSession(session = null) {
    try {
      ...
      return !this._isSessionExpired(targetSession);
    } catch (error) {
  ...
  clearSession() {
    try {
      ...
      this._notifyListeners('sessionCleared', null);
    } catch (error) {
  ...
  async refreshSession(session = null) {
    try {
      ...
      this._storeSession(refreshedSession);
      this._notifyListeners('sessionRefreshed', refreshedSession);
  ...
  async updateUser(userData) {
    try {
      ...
      const updatedSession = {
        ...this.currentSession,
        user: { ...this.currentSession.user, ...userData },
        updatedAt: new Date().toISOString()
      };

      this.currentSession = updatedSession;
      this._storeSession(updatedSession);
      this._notifyListeners('userUpdated', updatedSession.user);
  ...
  async createGuestSession() {
    try {
      ...
      const session = await this.createSession(guestUser, null);
      if (session) {
        session.isEmergencySession = true;
      }
  ...
  async getSessionInfo() {
    try {
      ...
      return {
        ...
        isEmergency: session.isEmergencySession || false
      };
    } catch (error) {
